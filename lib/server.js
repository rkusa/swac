...
exports.area = function(target, opts) {
  ...
  !function() {
    var src = null, lastModified = new Date()
    server.get(opts.mount, authorize, function(req, res) {
      if ('if-modified-since' in req.headers && +(new Date(req.headers['if-modified-since'])) === +lastModified) {
        res.status(304)
        return;
      }
...
function initialize() {
  ...
  commons.require(__dirname + '/../', { expose: 'swac' })
  commons.ignore(__dirname + '/helper/request.server.js')
  commons.ignore(__dirname + '/server.js')
  commons.require(__dirname + '/../node_modules/node-eventproxy', { expose: 'node-eventproxy' })
  commons.require(__dirname + '/../node_modules/implode', { expose: 'implode' })
