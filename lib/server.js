...
razorjs.attr = function(name, value) {
  return '"' + process.domain.app.attr(
    name,
    transformIntoRazorFunction(value)
  ).replace(/"/g, '\\"') + '"'
}

razorjs.inline = function(js) {
  return '"' + process.domain.app.block(
    transformIntoRazorFunction(js)
  ).replace(/"/g, '\\"') + '"'
}
