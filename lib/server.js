...
exports.middleware = function(target) {
  if (!fs.existsSync(target)) {
    throw new Error("Path " + target + " not found")
  }
  if (assetPath !== null) return undefined
  assetPath = target
  ...
  utils.wait()
  commons.external('swac')
  commons.ignore(__dirname + '/server.js')
  commons.bundle(function(err, src) {
    if (err) throw err
    fs.writeFile(path.join(assetPath, 'swac.js'), src, function(err) {
      if (err) throw err
      utils.ready()
    })
  })
...
exports.area = function(target, opts) {
  ...
  if (opts.allow || opts.deny) {
    ...
    // server.use(path.join(assetPath, opts.mount), authorize)
  }
  ...
  utils.wait()
  bundle.ignore('swac')
  bundle.ignore(__dirname + '/server.js')
  bundle.bundle(function(err, src) {
    if (err) throw err
    fs.writeFile(path.join(assetPath, opts.mount), src, function(err) {
      if (err) throw err
      utils.ready()
    })
  })

  // delayed loading of swac' main and routing functionality
  if (!routing) routing  = require('./routing')
  if (!swac)    swac = require('./')
