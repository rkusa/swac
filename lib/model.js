...
Definition.prototype.use = function(adapter, opts) {
  this.async = true
  if (!opts) opts = {}
  if (utils.isServer) {
    ...
    if (opts.serverOnly) return null
    adapter = require('./adapter/ajax')
  }
  api[this.name] = adapter.initialize(this.obj, opts, this.callback)
  var that = this
...
Model.define = function(name, opts, define, callback) {
  ...
  model.prototype.save = function(callback) {
    ...
    }) : api[this._type].put(this._id, props, function(err, res) {
      if (callback) callback(res)
    })
  }
  ...
  model.prototype.destroy = function(callback) {
    ...
    this.isNew ? cb() : api[this._type].delete(this._id, cb)
  }
...
function defineApi(model, method, opts) {
  ...
  [path].concat(scope || [], function(req, res) {
    var _fn = api[model._type][method]
