...
Model.define = function(name, definition) {
  ...
  model.prototype.property = function(key, opts) {
    var that = this
      , value = null
      , changed = function() {
        utils.debug('[Model] "%s"\'s Property "%s" got changed ... forwarding Event to its Model', that._position || name, key)
    ...
    Object.defineProperty(this, key, {
      ...
      set: function(newValue) {
        ...
        if (!opts.silent) {
          utils.debug('[Model] "%s"\'s Property "%s" got changed',that._position || name, key)
          that.emit('changed.' + key, that)
        }
      },
      enumerable: true
    })
