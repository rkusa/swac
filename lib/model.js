...
function authorize(model, method, allowFn, denyFn, allowed, denied) {
  ...
  return function() {
    ...
    }))) !== 'undefined') callback.apply(null, args)
...
Model.authorizeWrite = function(model, args, method, allowed, denied) {
  ...
  var first = args[0]
    , isAPICall = process.domain.isAPICall
  function callback(err, body) {
    if (err) throw err
    process.domain.isAPICall = isAPICall
    authorize(model, method, allow, deny, allowed, denied)(body)
  }
  if (method === 'post' || first instanceof model) {
    callback(null, first)
  } else {
    process.domain.isAPICall = false
...
Model.authorizeRead = function(model, method, models, callback) {
  ...
  utils.series(input, function(row, next) {
    authorize(model, method, allow, deny, function() {
      output.push(row)
      next()
    }, next)(row)
  }, function() {
  ...
  return function() {
    ...
    Model.authorizeWrite(
      ...
      function(body) {
        ...
        var data = args[0] instanceof model ? args[0].toJSON() : args[0]
        ...
        if (data && (method === 'post' || method === 'put')) {
          utils.series(Object.keys(data), function(key, next) {
            ...
            if ( !model.prototype._validation[key]
              ...
              !function(key) {
                authorize(model, method, allow, deny, next, function() {
                  delete data[key]
                  next()
                })(data, data[key], key)
              }(key)
