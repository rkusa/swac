...
Definition.prototype.apply = function(obj) {
  ...
  this.properties.forEach(function(key) {
    ...
    if ('default' in that.obj.prototype._validation[key]) {
      values[key] = typeof that.obj.prototype._validation[key].default === 'function'
        ? that.obj.prototype._validation[key].default()
        : that.obj.prototype._validation[key].default
    } else {
      ...
      set: function(newValue) {
        ...
        if (Array.isArray(newValue)) {
          ...
          if (!this._validation[key].silent) {
            ...
            values[key].on('changed', this, 'triggerChanged')
          }
        } else {
          values[key] = newValue
        }
...
Model.define = function(name, opts, define, callback) {
  ...
  var model = function(properties) {
    ...
    definition.properties.forEach(function(property) {
      if (that._validation[property].default) {
        that[property] = typeof that._validation[property].default === 'function'
          ? that._validation[property].default()
          : JSON.parse(JSON.stringify(that._validation[property].default)) // clone!
    ...
    if (properties) {
      Object.keys(properties).forEach(function(property) {
        if (definition.properties.indexOf(property) !== -1) {
  ...
  implode.register('Model/' + name, model,
                  ['id', 'events', 'isNew', '$errors', '$warnings'].concat(definition.properties))
  ...
  model.prototype.resetChangedStatus = function() {
    ...
    var that = this
    definition.properties.forEach(function(property) {
      that._changedValues[property] = undefined
    })
  }
  ...
  model.prototype.save = function(onSuccess, onError) {
    ...
    var callback = function(err, res) {
      that._saveState.inProgress = false
      if (err) {
        if (onError) return onError(err)
        else return onSuccess(err)
      }
      if (onSuccess) onSuccess(null, res)
      if (onSuccess = that._saveState.queue.shift()) {
        that.changed = true
        that.save(onSuccess)
      }
    }
  ...
  model.prototype.updateAttributes = function(attrs, opts) {
    ...
    Object.keys(attrs).forEach(function(key) {
      ...
      if (opts.silent) {
        var originalValue = that._validation[key].silent
        that._validation[key].silent = true
        that[key] = attrs[key]
        that._validation[key].silent = originalValue
  ...
  model.prototype.validate = function(properties) {
    ...
    var validation = (utils.isClient ? window.json : revalidator).validate(this, { properties: expectation }, { cast: true })
      , old = this.$warnings
    this.$warnings = {}
    validation.errors.filter(function(err) {
      return !properties || !!~properties.indexOf(err.property)
    }).forEach(function(err) {
      that.$warnings[err.property] = err
      that.emit('warnings.changed.' + err.property)
    })
    Object.keys(old).forEach(function(key) {
      if (!(key in that.$warnings))
        that.emit('warnings.changed.' + key)
    })
    
    old = this.$errors
    validation = (utils.isClient ? window.json : revalidator).validate(this, { properties: this._validation }, { cast: true })
    this.$errors = {}
    validation.errors.filter(function(err) {
      return !properties || !!~properties.indexOf(err.property)
    }).forEach(function(err) {
