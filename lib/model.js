...
Model.define = function(name, opts, definition, callback) {
  ...
  Model.prepareApi(model, 'list',   opts)
  Model.prepareApi(model, 'get',    opts)
  Model.prepareApi(model, 'post',   opts)
  Model.prepareApi(model, 'put',    opts)
  Model.prepareApi(model, 'delete', opts)

  var context = { property: function() {} }
    , async = false
  ...
  model.allow = {}
  context.allow = function(definition) {
    Object.keys(definition).forEach(function(prop) {
      model.allow[prop] = definition[prop]
    })
  }

  model.deny = {}
  context.deny = function(definition) {
    Object.keys(definition).forEach(function(prop) {
      model.deny[prop] = definition[prop]
    })
  }
  ...
  implode.register('Model/' + name, model, ['_id', 'events', 'isNew'])
  
  model.prototype.property = function(key, opts) {
    if (!opts) opts = {}
    this.$contract.push(key)
  ...
  model.prototype.allow = model.prototype.deny = function() {}
...
Model.prepareApi = function(model, method, opts) {
  if (!opts) opts = {}
  ...
  Object.defineProperty(model, method, {
    ...
    set: utils.isServer ? function(val) {
      fn = function() {
        ...
        Model.authorizeWrite(model, Model.currentRequest, method,
        // allowed
        function() {
          if (!callback) return
          args[args.length - 1] = function(err, result) {
            if (err) return callback(err)
            var output = Model.authorizeRead(model, Model.currentRequest, method, result)
            callback(null, output)
          }
          val.apply(model, args)
        },
        // denied
        function() {
...
Model.authorizeWrite = function(model, req, method, allowed, denied) {
  ...
  if (method === 'post') {
    ...
    model.get(req.params.id, function(err, body) {
      if (err) throw err
