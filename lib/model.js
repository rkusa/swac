...
Model.define = function(name, opts, define, callback) {
  ...
  model.prototype.validate = function(properties) {
    ...
    var validation = (utils.isClient ? window.json : revalidator).validate(this, { properties: expectation })
      , old = this.$warnings
    this.$warnings = {}
    ...
    old = this.$errors
    validation = (utils.isClient ? window.json : revalidator).validate(this, { properties: this._validation })
    this.$errors = {}
    validation.errors.filter(function(err) {
      return !properties || !!~properties.indexOf(err.property)
    }).forEach(function(err) {
