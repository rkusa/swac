...
Definition.prototype.use = utils.isServer ? function(adapter, opts) {
  this.async = true
  if (!opts) opts = {}
  if (typeof adapter === 'string')
    adapter = (require)('arkansas-' + adapter)
  return adapter.initialize.call(this.obj, this.name, opts, this.callback)
} : function() {}
...
Model.define = function(name, opts, define, callback) {
  ...
  // set _id if not defined through provided definition
  if (definition.properties.indexOf('_id') === -1)
    definition.property('_id')

  if (!definition.async && callback) callback()
  utils.eventify(model)
  implode.register('Model/' + name, model,
                  ['_id', 'events', 'isNew'].concat(Object.keys(definition.properties)))
