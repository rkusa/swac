...
Model.define = function(name, opts, define, callback) {
  ...
  model.prototype.toJSON = function(all, method, isAPICall) {
    ...
    authorizationContext.isClient = utils.isClient ? true : (isAPICall === true)
  ...
  model.prototype.validate = function(properties, callback) {
    ...
    var that = this, isValid = undefined
    ...
    validate(this, expectation, function(issues) {
      var old = that.$warnings
      that.$warnings = {}
      for (var prop in issues) {
        if (properties && !!!~properties.indexOf(prop)) continue
        that.$warnings[prop] = issues[prop]
        that.emit('warnings.changed.' + prop)
      }
      ...
      validate(that, that._validation, function(issues) {
        var old = that.$errors
        that.$errors = {}
        for (var prop in issues) {
          if (properties && !!!~properties.indexOf(prop)) continue
          that.$errors[prop] = issues[prop]
          that.emit('errors.changed.' + prop)
        }
        Object.keys(old).forEach(function(key) {
