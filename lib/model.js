...
function authorize(model, method, allowFn, denyFn, allowed, denied) {
  var authorizationContext = {}
...
// generic API Method
Model.getMethodFn = function(method, model) {
  return function() {
    ...
    if (utils.isClient) return fn.apply(Model.api[model._type], args)
    ...
    .chain(function(input, instance, done) {
      ...
      if (method !== 'delete' && Object.keys(input).length === 0)
        return done(instance)

      var data = input instanceof model ? input.toJSON() : input
      if (method === 'put')
        delete data.id
        ...
        function() {
          ...
          .chain(function(done) {
            ...
            utils.series(Object.keys(data), function(key, next) {
              ...
              if (
                ...
                // is serverOnly Property and got access through the Web Service
                (process.domain.isAPICall === true && model.prototype._validation[key].serverOnly)
              ) {
                delete data[key]
                return next()
              }
          ...
          .chain(function(done) {
            ...
            .chain(function(done) {
              ...
              if (!hook) return done()
              executeHook(model, input, 'beforeSave', function() {
                executeHook(model, input, hook, done)
              }) 
            })
            // Execute Write
            .chain(function(done) {
...
function defineApi(model, method, opts) {
  ...
  [path].concat(scope || [], function(req, res) {
    ...
    var _fn = Model.getMethodFn(method, model)
