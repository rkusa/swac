...
Model.define = function(name, definition) {
  ...
  model.prototype.property = function(key, opts) {
    var that = this
      , value = null
      , changed = function() {
        console.log('[Model] "' + (that._position || name) + '"\'s Property "' + key + '" got changed ... forwarding Event to its Model')
        ...
        if (Array.isArray(newValue)) {
          ...
          if (value && !value.hasOwnProperty('_position')) {
            Object.defineProperty(value, '_position', {
              value: that._position + '.' + key,
              writable: true
            })
          } else value._position = that._position + '.' + key
          if (!opts.silent) value.on('changed.*', changed)
        } else value = newValue
        if (!opts.silent) {
          console.log('[Model] "' + (that._position || name) + '"\'s Property "' + key + '" got changed')
          that.emit('changed.' + key)
        }
      },
      enumerable: true
  ...
  model.prototype.destroy = function(callback) {
    var that = this
      , cb = function() {
        ...
        console.log('[Model] "' + (that._position || name) + '" got destroyed')
        that.emit('destroy')
      }
