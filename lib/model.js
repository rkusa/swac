...
Model.authorizeWrite = function(model, args, method, allowed, denied) {
  ...
  if (method === 'post') {
    if (!((!allow || allow.call(authorizationContext, process.domain.req))
      && (!deny || !deny.call(authorizationContext, process.domain.req))))
      denied()
    else
      allowed()
  } else {
    ...
    function callback(err, body) {
      if (err) throw err
      if (!((!allow || allow.call(authorizationContext, process.domain.req, body))
        && (!deny || !deny.call(authorizationContext, process.domain.req, body))))
        denied()
      else
        allowed(body)
    }
...
Model.authorizeRead = function(model, method, models) {
  ...
  input.forEach(function(row) {
    if (((!allow || allow.call(authorizationContext, process.domain.req, row))
      && (!deny || !deny.call(authorizationContext, process.domain.req, row))))
      output.push(row)
  })
    ...
    Model.authorizeWrite(
      ...
      function(body) {
        ...
        args[args.length - 1] = function(err, result) {
          ...
          var output = result !== null ? Model.authorizeRead(model, method, result) : null
