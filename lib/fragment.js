...
Fragment.prototype.observe = function(model, propertyName) {
  ...
  if (model._position.indexOf('.') == -1) {
    ...
    this.on('detached', window.app.removeListener.bind(window.app, 'changed:' + model._position, refresh))
  }
  if (propertyName) {
    model.on('changed:' + propertyName, refresh)
    this.on('detached', model.removeListener.bind(model, 'changed:' + propertyName, refresh))
  } else {
    model.on('changed', this.refresh.bind(this))
    this.on('detached', model.removeListener.bind(model, 'changed', refresh))
  }
}
  ...
  Object.keys(events).forEach(function(path) {
    ...
    // } else {
      properties.forEach(function(property) {
        ...
        that.on('detached', model.removeListener.bind(model, event, refresh))
      })
    // }
    if (path.indexOf('.') == -1) {
      window.app.on('changed:' + path, refresh)
      that.on('detached', window.app.removeListener.bind(window.app, 'changed:' + path, refresh))
    }
  })
