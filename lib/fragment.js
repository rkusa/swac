...
require('./implode').register('Fragment', Fragment, ['id', 'template', 'context', 'events', 'parent', 'children', 'args', 'silent'])
...
Fragment.prototype.observe = function(model, property) {
  ...
  model.once('replaced', this, 'refresh')
  this.once('delete', model, 'off', model, 'replaced', this)
}

Fragment.prototype.observeError = function(model, property) {
  if (this.silent) return
  var event = 'errors.changed' + (property ? '.' + property : '')
  model.on(event, this, 'refresh')
  this.once('delete', model, 'off', model, event, this)
  model.once('replaced', this, 'refresh')
  this.once('delete', model, 'off', model, 'replaced', this)
...
Fragment.prototype.insert = function(context, index, type) {
  utils.debug('Insert into fragment #' + this.id + ' at ' + index)

  var fragment = new type(42, this.template, this.context, context)
  ...
  context.on('moved', this, 'move', this, fragment)
  fragment.once('delete', this, 'off', this, 'moved', this)
...
Fragment.prototype.move = function(child, index) {
  ...
  var node, next = child.startNode.nextSibling
  while ((node = next) !== child.endNode) {
    next = node.nextSibling
    before.parentNode.insertBefore(node, before)
  }
  after.nextSibling.parentNode.insertBefore(child.startNode, after.nextSibling)
  before.parentNode.insertBefore(child.endNode, before)
}
