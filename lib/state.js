...
State.prototype.attr = function attr(attrName, fn) {
  var argNames = attr.caller.fragment ? attr.caller.fragment.template.argNames.slice() : []
  ...
  attribute.args = attr.caller.fragment ? attr.caller.fragment.args.slice() : []
...
State.prototype.block = function block(fn, opts) {
  ...
  var argNames = block.caller.fragment ? block.caller.fragment.template.argNames.slice() : []
    , template = new Template(fn, argNames)
    , fragment = createFragment(this.original, template, this.original, block.caller.fragment)
  fragment.args = block.caller.fragment ? block.caller.fragment.args.slice() : []
...
State.prototype.collection = function collection(/* [tag, ]context, fn[, opts] */) {
  ...
  var argNames = collection.caller.fragment.template.argNames.concat(getArgNames(fn)[0])
  var template = new Template(fn, argNames)
    , list = createFragment(this.original, template, this.original, collection.caller.fragment)
  list.args = collection.caller.fragment.args.slice()
  if (tag) {
