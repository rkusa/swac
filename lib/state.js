...
var State = module.exports = function() {
  ...
  Object.defineProperties(this, {
    ...
    _sectionOrigins: { value: {}, writable: true },
...
implode.register('swac/State', State, ['fragments', 'sections', 'views', 'path', '_events', '_origins', '_sectionOrigins', '_path', 'nextFragmentId'])
...
State.prototype.register = function(name, obj, opts) {
  ...
  Object.defineProperty(this, name, {
    get: function get() {
      if (get.caller && typeof get.caller.fragment != 'undefined')
...
State.prototype.registerSection = function area(name, fn) {
  if (this._currentRoute) {
    if (!this._sectionOrigins[this._currentRoute]) this._sectionOrigins[this._currentRoute] = []
    this._sectionOrigins[this._currentRoute].push(name)
  }
...
State.prototype.attr = function attr(attrName, fn) {
  var argNames = (attr.caller.fragment ? attr.caller.fragment.template.argNames.slice() : []).concat(attr.caller.argNames || [])
    , template = new Template(fn, argNames)
    , attribute = createFragment(this.original, template, this.original, attr.caller.fragment, Attribute)
  attribute.args = (attr.caller.fragment ? attr.caller.fragment.args.slice() : []).concat(attr.caller.args || [])
  var ret = 'data-bind-' + attrName + '="' + attribute.id + '" '
  ret += attrName + '="' + attribute.render() + '"'
  return ret
}

State.prototype.block = function block(fn, opts) {
  if (!opts) opts = {}

  var argNames = (block.caller.fragment ? block.caller.fragment.template.argNames.slice() : []).concat(block.caller.argNames || [])
    , template = new Template(fn, argNames)
    , fragment = createFragment(this.original, template, this.original, block.caller.fragment)
  fragment.args = (block.caller.fragment ? block.caller.fragment.args.slice() : []).concat(block.caller.args || [])
  if (typeof opts !== 'object') opts = {}
  if (opts.silent === true) fragment.silent = true
  return fragment.wrap(fragment.render())
}

State.prototype.collection = function collection(/* [tag, ]context, fn[, opts] */) {
  var that = this
  ...
  var argNames = (collection.caller.fragment ? collection.caller.fragment.template.argNames : []).concat(collection.caller.argNames || [], getArgNames(fn)[0])
  var template = new Template(fn, argNames)
    , list = createFragment(this.original, template, this.original, collection.caller.fragment)
  list.args = (collection.caller.fragment ? collection.caller.fragment.args.slice() : []).concat(collection.caller.args || [])
  if (tag) {
