...
var State = module.exports = function() {
  ...
  var self = this
  proxy.on('set', function(path, isUpdate) {
    if (isUpdate || !self.current) return
    if (!self.origins[self.current]) {
      self.origins[self.current] = []
    }
    self.origins[self.current].push(path)
  })
}

var implode = require('implode')
implode.register('swac/State', State, ['currentView', 'templates', 'fragments', 'locals', 'observer', 'location', 'origins'])
