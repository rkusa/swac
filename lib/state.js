...
implode.register('swac/State', State, ['fragments', 'areas', 'views', 'path', 'events'])
...
State.prototype.area = function area(name, fn) {
  ...
  area[name] = function() {
    ...
    if (!(that.currentView in that.views)) that.views[that.currentView] = []
    that.views[that.currentView].push({
      area: name,
...
State.prototype.collection = function(/* context, [args,] fn[, opts] */) {
  var that = this
