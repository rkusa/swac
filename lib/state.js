...
State.prototype.collection = function collection(/* [tag, ]context, fn[, opts] */) {
  ...
  var argNames = (collection.caller.fragment.template.argNames.concat(collection.caller.argNames || [], getArgNames(fn)[0]))
  var template = new Template(fn, argNames)
    , list = createFragment(this.original, template, this.original, collection.caller.fragment)
  list.args = collection.caller.fragment.args.slice().concat(collection.caller.args || [])
  if (tag) {
