...
exports.Array = function(values, model, trackPositions) {
  if (!model) {
    if (typeof values === 'boolean') {
      trackPositions = values
      values = null
    } else if (!Array.isArray(values)) {
      model = values
      values = null
    }
  } else if (!trackPositions && typeof model === 'boolean') {
    trackPositions = model
    model = null
  }
  ...
  var contract = ['model', 'trackPositions']
  contract.id = 'ObservableArray'
  Object.defineProperty(array, '$contract', {
    value: contract
  })
  array.model = model
  array.trackPositions = trackPositions
    ...
    if (typeof element === 'object') {
      if (trackPositions) {
        if (!element.hasOwnProperty('_index')) {
          Object.defineProperty(element, '_index', {
            value: null,
            writable: true,
            configurable: true
          })
        ...
        Object.defineProperty(element, '_position', {
          get: function() {
            return array._position + '.' + (this._index + positionModifier)
          },
          set: function(val) {
            this._index = val.split().pop()
          }
        })
      }
  ...
  array.push = function() {
    ...
    elements.forEach(function(element) {
      ...
      added(element)
      if (trackPositions)
        element._index = that.length - 1 + (positionModifier * -1)
    })
    this.emit('changed')
    return result
  }
  ...
  array.unshift = function() {
    ...
    elements.forEach(function(element) {
      ...
      added(element)
      if (trackPositions) {
        element._index = (positionModifier * -1) - 1
        ++positionModifier
      }
    })
    this.emit('changed')
...
new Contract('ObservableArray', exports.Array, ['model', 'trackPositions'])
