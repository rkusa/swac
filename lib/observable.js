  ...
  function adding(element) {
    ...
    if (element._id) {
      ...
      if (gotEventified(element)) {
        element.on('changed._id', array, 'reassign', array, element)
        element.once('removed', element, 'off', element, 'changed._id', array, 'reassign')
      }
    }
    return element
  }
  ...
  array.push = function() {
    ...
    if (!this.compareFunction) {
      ...
      elements.forEach(function(element) {
        ...
        // this have to be appended to the event queue
...
exports.GroupedArray = function(model, prop, sub) {
  ...
  grouped.createGroup = function(key) {
    var group = new Group({
