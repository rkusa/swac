...
var route = function(method, path, action, rdy, options) {
  ...
  var leaf = function(app, cb, req, res) {
    ...
    // if (utils.isBrowser && initialized && app.path.pattern == req.pattern
  ...
  var callback = function(req, res) {
    ...
    req.pattern = path
    var app = utils.isBrowser ? window.app : new State
      , done
    ...
    if (utils.isBrowser) {
        ...
        var render = function(result) {
          ...
          // 
          //   utils.aquireFragments(root.DOMRange.commonAncestorContainer, result.fragments, function(node) {
  ...
  if (utils.isServer) server[method](path, callback)
  ...
  return {
    ...
    delete: route.bind({ parent: leaf, parentRoute: about }, 'delete')
  }
}
...
exports.delete = route.bind(rootContext, 'delete')