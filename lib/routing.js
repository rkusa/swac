    ...
    if (utils.isClient) {
      ...
      if (initialized && !app.path.traversing) {
        ...
        do {
          ...
          if (relevant === 0 || irrelevant === 0) break
          var lhs = app.path.requested.replace(/^\/_(post|put|delete)\//, '/').split('/').splice(irrelevant + 1, relevant)
  ...
  var callback = function(req, res) {
    ...
    if (utils.isServer) {
      ...
      if (req.session && req.session.messages) {
        // TODO
        // app.flashs.reset(req.session.messages.map(function(msg) {
        //   return new State.Flash(msg)
        // }))
    ...
    if (utils.isClient) {
          ...
          viewNames.forEach(function(viewName, i) {
            ...
            for (var sectionName in views[viewName]) {
              ...
              if (fragment && fragment.startNode) {
                ...
                app.registerSection(sectionName, template.source)
              }
            }
          })
          if (initialized && rdy) {
      ...
      // express render and redirect
        ...
        if (req.session) {
          // TODO
          // req.session.messages = app.flashs.map(function(flash) {
          //   return flash.toJSON()
          // })
        }
        ...
        utils.series(views, function(viewName, next, remaining) {
          ...
          exports.server.app.render(viewName, app, function(err) {
            if (err) throw err
