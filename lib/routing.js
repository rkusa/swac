...
var route = function(conf, method, path, action, rdy, options) {
  ...
  var attach = function() {
    ...
    if (utils.isServer) server[method](path, conf.allow, callback)
...
exports.using = function(allow, opts) {
  ...
  if (!opts) opts = {}
  var conf = {
    allow: allow || function(req, res, next) {
      next()
    },
  ...
  return {
    get:    route.bind(rootContext, conf, 'get'),
    post:   route.bind(rootContext, conf, 'post'),
    put:    route.bind(rootContext, conf, 'put'),
    delete: route.bind(rootContext, conf, 'delete'),
  }
}
