...
var route = function(method, path, action, rdy, options) {
  ...
  if (isBrowser && method != 'get') path = '/' + method + path

  // dispatch route
  if (isBrowser) page(path, bla)
  else server[method](path, bla)
