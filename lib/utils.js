...
Observer.prototype.on = function(target, method) {
  target.off(this.event, proxy(this.target, this.method))
  target[method || 'on'](this.event, proxy.apply(null, [this.target, this.method, this.target].concat(this.args)))
  if (!this.until) return
  (this.untilTarget || target).off(this.until, proxy(target, 'off'));
  (this.untilTarget || target).once(this.until, proxy(target, 'off', target, this.event, proxy(this.target, this.method)))
}
