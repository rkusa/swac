  ...
  describe('Model', function() {
    ...
    describe('all', function() {
      ...
      it('should work using GET', function(done) {
        async.series([
          function(cb) {
            fixtures.client.get('/_api/todo/' + a._id)
            .end(function(err, res) {
              res.status.should.equal(200)
              var obj = JSON.parse(res.text)
              obj.should.have.property('_id', a._id)
              cb()
            })
          },
          function(cb) {
            fixtures.client.get('/_api/todo/' + b._id)
            .end(function(err, res) {
              res.status.should.equal(200)
              var obj = JSON.parse(res.text)
          ...
          function(cb) {
            Todo.get(a._id, function(err, todo) {
              todo.should.have.property('_id', a._id)
              cb()
            })
          },
          function(cb) {
            Todo.get(b._id, function(err, todo) {
        ...
        async.series([
          function(cb) {
            ...
            .end(function(err, res) {
              ...
              obj[0].should.have.property('_id', a._id)
              cb()
            })
          },
          function(cb) {
            Todo.list(function(err, todos) {
              ...
              todos[0].should.have.property('_id', a._id)
              cb()
            })
          }
        ], done)
